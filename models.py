...
def get_ticket_by_code(ticket_code: str) -> Optional[Dict]:
    ...
    if row is None:
        return None
    return dict(row)
def redeem_ticket_atomic(ticket_code: str) -> bool:
    """
    Atomically transition ISSUED -> REDEEMED.
    Returns True if we successfully redeemed the ticket (was ISSUED),
    False if it was missing or already redeemed/used.
    """
    with db.db_cursor() as cur:
        cur.execute(
            """
...
def set_ticket_status(ticket_code: str, status: str) -> None:
    with db.db_cursor() as cur:
        cur.execute(
