...
(() => {
  ...
  function bootWorld() {
    ...
    camera.position.set(0, 1.95, 4.95);
  ...
  function updateCamera(dt) {
    ...
    if (cam.mode === "enter") {
      ...
      x = sw;
      z = 5.05 + Math.cos(cam.t * 0.26) * 0.14;
      y = 1.92 + Math.sin(cam.t * 0.4) * 0.03;
    } else if (cam.mode === "aim" && run && run.aim) {
      x = run.aim.yaw * 0.28;
      y = 1.92 + run.aim.power * 0.06;
      z = 4.95 + run.aim.power * 0.2;
      lx = Math.sin(run.aim.yaw) * 0.45;
      ly = 0.86 + run.aim.pitch * 0.22;
    } else if (cam.mode === "throw" && run && run.ball) {
      x = lerp(camera.position.x, run.ball.x * 0.15, 0.08);
      y = lerp(camera.position.y, 1.85 + run.ball.y * 0.08, 0.08);
      z = lerp(camera.position.z, 4.55, 0.06);
      lx = run.ball.x * 0.3;
      ly = run.ball.y;
      lz = run.ball.z;
