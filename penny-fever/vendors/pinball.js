...
import { PinballFraming } from "./pinball-camera.js";
...
function mountStall(PF) {
  ...
  function ensureGL() {
    ...
    camera.position.set(0.55, 2.05, -3.55);
    ...
    gl.framing = new PinballFraming(camera, gl.cabinet, canvas.parentElement);
  ...
  function resizeGL() {
    ...
    if (gl.framing) gl.framing.resize(w, h);
  }
  ...
  function aimCamera(dt) {
    ...
    gl.cam.mode = mode;
    if (gl.framing && gl.framing.update(dt, mode)) {
  ...
  // Preserved for the explicit Compare old button; it no longer overrides wide view.
  function aimLegacyCamera(dt) {
    if (!gl) return;
    const cam = gl.camera;
    const mode = (run && !run.done) ? (run.ball && run.ball.mode === "plunger" ? "plunger" : run.draining ? "drain" : "play") : (run && run.done ? "result" : "vestibule");
    gl.cam.mode = mode;
  ...
  PF.registerVendor({
    ...
    onLeave() {
      visible = false;
      if (gl && gl.framing) gl.framing.controls.querySelector("details").open = false;
