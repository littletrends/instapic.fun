import { WORLD, SHORE, COURSES, JET_RANGE, TREASURE_ART, TREASURE_NAMES, gatePose, ferryPose, allLetters } from './model.js';
...
export class HarbourScene {
  constructor(canvas) {
    ...
    for (const [id, src] of Object.entries(TREASURE_ART)) {
      if (typeof Image === 'undefined') { this.art[id] = { naturalWidth: 0 }; continue; }
  ...
  treasure(x, y, i, t, collected, reduced, id) {
    if (collected) return;
    const c = this.ctx, bob = reduced ? 0 : Math.sin(t * 2.4 + i) * 3;
    this.ellipse(x, y + 12, 28, 10, '#16576135');
    this.ellipse(x, y + 12, 34 + Math.sin(t * 2 + i) * 3, 12, null, '#d4fff280', 1.5);
    c.save(); c.translate(x, y + bob); c.rotate(Math.sin(t + i) * .05);
    const image = this.art[id];
    if (image && image.naturalWidth) {
      const wide = id === 'return-postcard' || id === 'message-bottle';
      const w = wide ? 58 : 46, h = wide ? 58 : 46;
      c.drawImage(image, -w / 2, -h / 2, w, h);
    } else {
      this.ellipse(0, 0, 22, 22, '#7b2743', '#e8c075', 3);
    }
    c.restore();
  }
  dock(g, t, reduced) {
    ...
    c.restore();
    this.label(unlocked ? 'POCKET HERE' : 'COLLECT ALL 3', x, y - 69, 17, '#fff4d6');
    if (g.dockTime > 0) {
  ...
  effects(t, reduced) {
    ...
    for (const r of this.rings) {
      ...
      if (r.type === 'treasure') this.label((TREASURE_NAMES[r.id] || 'KEEPSAKE') + '!', r.x, r.y - 30 - age * 30, 15, '#fff2cc');
  ...
  render(g, { time = 0, pointer = null, reduced = false } = {}) {
    ...
    course.letters.forEach(([x, y], i) => objects.push({ y, draw: () => this.treasure(x, y, i, time, g.letters[i], reduced, course.finds[i]) }));
    ...
    if (g.phase === 'ready') {
      this.label('THE LITTLE SAILBOAT', g.boat.x, g.boat.y + 57, 18, '#fff1cc');
