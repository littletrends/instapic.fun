import { WORLD, COURSES, TREASURE_NAMES, createGame, startGame, step, allLetters } from './model.js';
...
function sync() {
  ...
  $('letters').textContent = `${game.letters.filter(Boolean).length} / 3 treasures`;
...
function handleEvents() {
  for (const e of game.events) {
    ...
    if (e.type === 'treasure') {
      const name = TREASURE_NAMES[e.id] || 'keepsake';
      say(allLetters(game) ? `All three aboard! Take them to the golden berth — that’s the pocket.` : `${name} aboard. Find the next floating treasure.`, 4);
    }
    ...
    if (e.type === 'won') {
      ...
      $('result-copy').textContent = [
        'Three pennies for the pocket book. The Little Sailboat came home dry.',
        'Moon penny and postcard rescued from the tide. Still a playtest — not saved yet.',
        'The message bottle is aboard. A future curio, practising for Your pocket.',
      ][game.course];
      $('result-stats').textContent = `${Math.round(game.time)} seconds · ${game.bumps} gentle ${game.bumps === 1 ? 'bump' : 'bumps'}. One seal for delivery, one for at most 4 bumps, one for ${COURSES[game.course].par}s or less. Playtest only — nothing is written to Your pocket yet.`;
...
function guidance() {
  ...
  else if (game.dockTime > .02) say('That’s it. Hold steady while the treasures go into the pocket…');
