...
function mountStall(PF) {
  ...
  function ensureWorld() {
    ...
    camera.position.set(0.15, 1.4, 4.2);
  ...
  function tickCamera(dt, t) {
    ...
    const live = isLive();
    let tx = 0.08; let ty = 1.34; let tz = 4.05;
    let lx = 0; let ly = 1.2; let lz = -1.0;
    if (!live) {
      const u = t * 0.00022;
      tx = Math.sin(u) * 1.2;
      ty = 1.48 + Math.sin(u * 1.3) * 0.08;
      tz = 3.95 + Math.cos(u * 0.8) * 0.5;
    } else if (run.aim) {
      tz = 4.18 + run.aim.power * 0.28;
      ty = 1.3;
      cam.fov += (48 - cam.fov) * 0.08;
    } else if (run.ball) {
      tx = run.ball.x * 0.22;
      ty = kit.lerp(1.34, run.ball.y + 0.45, 0.35);
      tz = kit.lerp(4.05, run.ball.z + 1.55, 0.28);
      lx = run.ball.x; ly = run.ball.y; lz = run.ball.z;
      cam.fov += (54 - cam.fov) * 0.08;
    } else cam.fov += (52 - cam.fov) * 0.08;
