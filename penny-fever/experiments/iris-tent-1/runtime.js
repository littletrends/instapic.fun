...
function tellRoom(type) { if (embedded) window.parent.postMessage({channel:"pf-paper-world", type, id, title:"Catch the Fortune"}, location.origin); }
...
try {
  ...
  const next = games.slice(games.indexOf(entry) + 1).find(g => g.ready);
  if (next) {
  ...
  if (!disposed) {
    ...
    if (new URLSearchParams(location.search).get("auto") === "1") {
      start();
      engine.action?.(state, "gaze", true, input);
      paint();
    }
  }
} catch (e) {
