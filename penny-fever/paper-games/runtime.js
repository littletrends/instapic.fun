...
function reset(){stop();ended=false;time=0;state=engine.create(level,seeded(1703+level*297));paint();$('#readout').textContent=engine.readout?.(state)||'';if(embedded&&engine.live)veil(entry.host+' presents','The trays are as you left them.','Drop a penny to wake the pushers. A rare on the lip waits. Leave and come back — the trays stay.','Step inside');else veil(entry.host+' presents',engine.levels[level],engine.instructions,'Begin chapter');$('#begin').disabled=false;$('#pause').textContent='Pause';}
...
function tick(now){if(!playing||disposed)return;try{if(!last)last=now;const dt=Math.min(.05,(now-last)/1000);last=now;time+=dt;engine.update?.(state,dt,input);if(now-paintAt>1000/30){paintAt=now;paint();}if(now-reportAt>350){reportAt=now;const text=engine.readout?.(state)||'';if($('#readout').textContent!==text)$('#readout').textContent=text;}if(state.result){persist();if(engine.live){delete state.result;raf=requestAnimationFrame(tick);return;}ended=true;stop();paint();const r=state.result,list=engine.prizes||kits[entry.id]?.prizes||[],prize=list[level]||list[list.length-1];veil('Chapter complete',r.title,r.detail,level<engine.levels.length-1?'Next chapter':'Play chapter again',prize);if(prize)tellRoom('prize',{item:prize,stall:entry.id,chapter:level});$('#readout').textContent=engine.readout?.(state)||'';return;}raf=requestAnimationFrame(tick);}catch(e){error(e);}}
...
try{
 ...
 engine=(await import(entry.module+'?v=paper-cashdrop-6')).default;
 ...
 listen($('#chapter'),'change',()=>{if(engine.live)return;level=Number($('#chapter').value);reset();});listen($('#restart'),'click',()=>{if(engine.live){persist();start();return;}reset();});listen($('#pause'),'click',()=>playing?pause():start());
 listen($('#begin'),'click',()=>{if(engine.live){start();return;}if(ended&&level<engine.levels.length-1){level++;$('#chapter').value=level;reset();}start();});
 ...
 const img=$('#backdrop');img.src=entry.asset;try{await img.decode();}catch{throw Error('The background artwork is missing or could not load.');}if(!disposed){reset();for(const id of ['chapter','pause','restart'])$('#'+id).disabled=false;if(engine.live){$('#chapter').disabled=true;$('#chapter').hidden=true;$('#restart').hidden=true;const lab=document.querySelector('label[for="chapter"]');if(lab)lab.hidden=true;}tellRoom('ready',{title:engine.title});}
}catch(e){error(e);}
