...
try{
 ...
 engine=(await import(entry.module+'?v=paper-cashdrop-8')).default;
 ...
 if(embedded){const note=document.querySelector('.note');if(note)note.textContent=entry.id==='coin-pusher'?'Three trays. Drop a penny or dump the pocket. The machine sleeps until you drop, and the trays are saved when you leave. Cash a booth ticket for a five-penny stack.':entry.id==='pinball'?'A penny pulls the plunger. Tap the flippers. Small wins drip back; rare treasures almost never leave the glass. Cash a booth ticket for a five-penny stack.':'A booth ticket from Aura’s roll starts this stall. Cash tickets for pennies at Copper Falls. Workshop play from the paper-games list stays free.';}
