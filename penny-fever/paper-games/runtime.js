...
const PHONE_PROMPTS={"fortune": "Gaze to begin. Brake when the rings line up.", "love": "Begin a sitting. Fill the slips using the clues.", "curios": "Open a mystery. Read the clue and tap a drawer.", "lookup": "Turn the pieces to connect the lantern path.", "whisper": "Start running. Jump to deliver the letters.", "ball-toss": "Sit down. Drag to aim, then release your ring.", "coin-pusher": "Tap Drop to add a penny. Watch the moving trays.", "pinball": "Hold Plunge, then release. Use both flippers to keep the ball moving.", "water-gun": "Sit down. Aim at the boat and hold to build pressure.", "milk-bottles": "Sit down. Drag to aim and release to throw.", "cover-the-spot": "Watch the spot. Follow it, then tap its hiding place.", "mutoscope": "Crank the reel. Brake to line up the picture.", "high-striker": "Sit down. Hold to pull, then release.", "catoptromancy": "Shatter the glass. Turn and place the shards.", "bent-rings": "Start a wave. Move the launcher and hold Toss to throw.", "plinko": "Drop a marble. Use the flippers to guide it.", "fairy-floss": "Spin the cloud. Turn left or right to shape it.", "popcorn": "Start the run. Use the arrows to steer.", "duck-pond": "Start the parade. Guide the ducks through the pond.", "skee-ball": "Step up. Aim, then tap Roll.", "penny-pitch": "Step up and Pitch. Tug left or right to guide the penny.", "dunk-tank": "Open the works. Start the pump, then drop the chair.", "marquee": "Serve to begin. Guide the light along the boardwalk.", "pack": "Open the case. Move, rotate and drop each piece.", "pass": "Cue the run. Use the arrows to move.", "carousel": "Drag to look around. Tap glints to collect three keepsakes.", "organ": "Watch the keys. Tap the same sequence back.", "helter": "Swipe left or right. Follow the painted arrow.", "ferris": "Drag the lens over a glint. Hold it steady, then tap.", "swings": "Hold Lean to swing out. Release to drift in toward the ring.", "funhouse": "Read the clue. Tap the matching door.", "balloons": "Hold for lift; release to descend. Fly through four arches.", "mural": "Choose two colours. Hold Paint when the stencil enters the frame."};
const HOUSE={
...
try{
 ...
 engine=(await import(entry.module+'?v=template-art-2')).default;
 document.title=engine.title+' · Penny Fever';$('#title').textContent=engine.title;$('#host').textContent=entry.host+'’s paper world';$('#compact-title').textContent=engine.title;$('#intro').textContent=engine.intro;$('#instructions').textContent=PHONE_PROMPTS[entry.id]||engine.instructions;canvas.setAttribute('aria-label',engine.title+'. '+engine.instructions);
 ...
 const img=$('#backdrop');img.src=entry.asset;
 try{await img.decode();}catch{throw new Error('The illustrated game background could not load. Please reload to try again.');}
