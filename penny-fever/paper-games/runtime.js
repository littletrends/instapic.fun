...
import {kits,spriteKey,itemName} from './prizes.js?v=chapter-bonus-19sep';
...
const HOUSE={
 ...
 love:{seconds:100,title:'The tester slept',detail:'Rosalie blots the page. Another sitting when you are ready.'},
 curios:{seconds:100,title:'The cabinet closed',detail:'The drawers latch. Another mystery when you have a penny.'},
 lookup:{seconds:100,title:'The sky went quiet',detail:'Celeste covers the lantern. Another sitting when you are ready.'},
 snap:{seconds:100,title:'The shutter slept',detail:'Felix winds a fresh plate. Try this woodland again.'},
 whisper:{seconds:100,title:'The whisper slept',detail:'Willa folds the last envelope. Another run when you are ready.'},
 'ball-toss':{seconds:100,title:'The lanterns stay lit',detail:'Bess collects the rings. One more penny for another toss.'},
 'coin-pusher':{seconds:100,title:'Copper banks the trays',detail:'The falls sleep until the next penny. This shift is over.'},
 pinball:{seconds:100,title:'The table went dark',detail:'Pip kills the lights. Plunge another penny when you want the garden back.'},
 'water-gun':{seconds:100,title:'The harbour stills',detail:'Marina ties the boats. Try the channel again.'},
 'milk-bottles':{seconds:100,title:'The dairy closes',detail:'Mabel restacks the bottles. Another penny, another throw.'},
 'cover-the-spot':{seconds:100,title:'The moons drifted',detail:'Dot gathers the discs. Cover them again next go.'},
 mutoscope:{seconds:100,title:'The reel ran out',detail:'The picture never quite found its last frame.'},
 'high-striker':{seconds:100,title:'The bronze stays quiet',detail:'Magnus resets the hammer. Ring it on the next penny.'},
 catoptromancy:{seconds:100,title:'The gardens fogged',detail:'Opal polishes the glass. Step through again when it clears.'},
 'bent-rings':{seconds:100,title:'The orchard emptied',detail:'Ringo collects the rings. Another toss when you are ready.'},
 plinko:{seconds:100,title:'The mill stopped',detail:'Peggy locks the gates. Drop another marble next chapter.'},
 'fairy-floss':{seconds:100,title:'The spinner went quiet',detail:'Flossie winds a fresh cloud. Try the atelier again.'},
 popcorn:{seconds:100,title:'The kettle slept',detail:'Poppy banks the kernels. The symphony waits for another penny.'},
 'duck-pond':{seconds:100,title:'The parade went home',detail:'Dottie gathers the ducklings. Lead them again next go.'},
 'skee-ball':{seconds:100,title:'The moonbow faded',detail:'Skip racks the balls. One more penny for another roll.'},
 'penny-pitch':{seconds:100,title:'The wells went still',detail:'Penelope scoops the pennies. Skip them again next chapter.'},
 'dunk-tank':{seconds:100,title:'The seat stays dry',detail:'Duncan wrings the splash. Aim again when you are ready.'},
 marquee:{seconds:100,title:'The boardwalk dimmed',detail:'Lumi kills the wave. Conduct the lights on the next penny.'},
 pack:{seconds:100,title:'The suitcase snapped shut',detail:'Kit latches the lid. Pack again when you have another penny.'},
 pass:{seconds:100,title:'The curtain fell',detail:'Bea closes the fly loft. Slip through again next call.'},
 carousel:{seconds:100,title:'The waltz ended',detail:'The lantern dimmed before you caught enough treasures.'},
 balloons:{seconds:100,title:'The bunch drifted off',detail:'Nell is tying the next handful. Try this garden again.'},
 ferris:{seconds:100,title:'The wheel slowed',detail:'The crescent waited, and the cabins went home.'},
 helter:{seconds:100,title:'The mat is empty',detail:'The slide ran out before every gold ring was caught.'},
 swings:{seconds:100,title:'The chairs emptied',detail:'The front mat waited, and the flight ended.'},
 funhouse:{seconds:100,title:'The mirrors went dark',detail:'The real laugh hid in the shuffle.'},
 organ:{seconds:100,title:'The roll finished',detail:'A few notes wandered off the gold bar.'},
 mural:{seconds:100,title:'The paint dried',detail:'A few patches still forget the alley wall.'},
};
const HOUSE_FALLBACK={seconds:100,title:'The house closes',detail:'This go is over. Try the chapter again.'};
function houseSpec(){
...
function paintHud(){
 ...
 if(cash){
  ...
  else cash.textContent=(n? (n+' '+(n===1?'penny':'pennies')) : 'Ready');
 }
 if(keep){
  const prize=chapterKeep();
...
function reset(){stop();ended=false;time=0;state=plantChapter(engine.create(level,seeded(1703+level*297)));if(state)state.practice=false;const house=houseSpec();if(state&&house)state.houseLeft=house.seconds;paint();$('#readout').textContent=engine.readout?.(state)||'';markChapters();const name=engine.levels[level];veil('','','','Begin');$('#begin').disabled=false;$('#pause').textContent='Pause';}
...
function swallowWin(r){
 ...
 const prize=resultPrize(r, r.won!==false);
 if(r.won===false||!prize)return false;
 takePrize(state,prize);
 delete state.result;
 return true;
}
function tick(now){if(!playing||disposed)return;try{if(!last)last=now;const dt=Math.min(.05,(now-last)/1000);last=now;time+=dt;engine.update?.(state,dt,input);const house=houseSpec();if(house&&state&&!state.result){if(state.houseLeft==null)state.houseLeft=house.seconds;state.houseLeft-=dt;if(state.houseLeft<=20)state.houseWinding=true;const settling=!!(state.settle>0||state.falling?.length||state.coins?.some?.(c=>c.falling));if(state.houseLeft<=0){if(settling&&(state.houseGrace=(state.houseGrace||0)+dt)<12){state.houseLeft=0.01;}else{state.houseLeft=0;state.result={title:house.title,detail:house.detail,won:false};}}}stepPrize(state,dt);if(state.prizeDeliver){state.prizeDeliver=false;handPrize();}if(now-paintAt>1000/30){paintAt=now;paint();}if(now-reportAt>350){reportAt=now;const text=engine.readout?.(state)||'';if($('#readout').textContent!==text)$('#readout').textContent=text;}if(state.result){persist();if(swallowWin(state.result)){raf=requestAnimationFrame(tick);return;}if(engine.live&&engine.chapterEnds===false&&state.result.won!==false){delete state.result;raf=requestAnimationFrame(tick);return;}ended=true;stop();paint();const r=state.result,won=r.won!==false,prize=resultPrize(r,won),stay=r.advance===false;veil(won?'Chapter complete':(engine.loseTitle||'Not this time'),r.title,r.detail,won?(stay?'Board the paid ride':(level<engine.levels.length-1?'Next chapter':'Play chapter again')):(engine.retryButton||'Try this chapter again'),won?prize:null);$('#readout').textContent=engine.readout?.(state)||'';return;}raf=requestAnimationFrame(tick);}catch(e){error(e);}}
...
try{
 ...
 engine=(await import(entry.module+'?v=love-layout-2')).default;
