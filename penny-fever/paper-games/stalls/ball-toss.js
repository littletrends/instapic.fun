...
function throwBall(s) {
  if (s.ball || s.cooldown > 0) return;
  s.ball = {...start, vx: (s.aim.x - start.x) / flight, vy: (s.aim.y - start.y - .5 * g * flight * flight) / flight};
  s.throws++;
  s.trail = [];
}
function lanternId(level, i) {
  return level && i % 2 ? 'moon-lantern' : 'autumn-leaf-lantern';
}
...
export default {
  ...
  update(s, dt, input) {
    ...
    for (const [i, l] of s.lanterns.entries()) {
      if (!l.fallen) {
        ...
        l.vy += 450 * dt; l.x += l.vx * dt; l.y += l.vy * dt; l.spin += l.vx * .006 * dt;
    ...
    if (s.ball) {
      ...
      s.trail.push({...p}); if (s.trail.length > 30) s.trail.shift();
      const hit = s.lanterns.find(l => !l.fallen && segmentDistance(l, old, p) < 42);
      if (hit) {
        hit.fallen = true; hit.vx = p.vx * .35; hit.vy = -80;
        p.vx *= -.45; p.vy *= .15; s.note = 'Down it comes!';
        if (s.lanterns.every(l => l.fallen)) s.cooldown = 1.4;
      }
      if (p.y > 1090 || p.y < 300 || p.x < 150 || p.x > 750) s.ball = null;
    }
    if (s.lanterns.every(l => l.fallen) && s.cooldown === 0) {
      done(s, 'The lanterns have landed', s.lanterns.length + ' lanterns in ' + s.throws + ' throws. Bess is already tying the next ribbons.');
    }
  },
  pointer(s, type, p) {
  ...
  draw(s, d) {
    ...
    d.path(s.trail, '#f8e6bd88', 4);
    if (s.ball) d.ball(s.ball.x, s.ball.y, 15, '#b57b59');
    else {
      const vx = (s.aim.x - 450) / flight, vy = (s.aim.y - 1010 - .5 * g * flight * flight) / flight;
      for (let i = 1; i < 15; i++) {
        const t = i * flight / 14;
        d.circle(450 + vx * t, 1010 + vy * t + .5 * g * t * t, 2.5, '#805946aa');
      }
      d.ball(450, 1010, 19, '#bc6f55');
