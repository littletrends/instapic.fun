...
function tap(s) {
  if (!s.coin) {
    s.coin = {x: 450, y: 1000, z: 10, vz: 130, vx: 0, touch: 0, skips: 0};
    s.hits = 0; s.wells.forEach(w => w.hit = false); s.runs++;
    s.note = 'Watch the penny meet its shadow.';
  }
  s.lastTap = s.t;
}
function sink(s, note) {
  s.ripples.push({x: s.coin.x, y: s.coin.y, t: 0});
  s.coin = null; s.note = note;
}
...
export default {
  ...
  update(s, dt, input) {
    ...
    c.vx += (axis * 200 - c.vx) * Math.min(1, dt * 5);
    if (c.touch > 0) {
      if (s.t - s.lastTap < .25) { c.touch = 0; c.vz = 130; c.z = 1; c.skips++; s.lastTap = -9; s.note = 'A clean little skip.'; }
      else {
        c.touch -= dt;
        if (c.touch <= 0) { sink(s, 'It sank gently. Tap a little closer to the water next run.'); return; }
      }
    } else {
      c.x = clamp(c.x + c.vx * dt, 200, 700); c.y -= 155 * dt; c.vz -= 260 * dt; c.z += c.vz * dt;
      if (c.z <= 0) {
        ...
        } else s.note = 'That bounce missed a ring. Keep going, or send a fresh penny after this run.';
      }
    }
    if (c.y < 360 || c.skips > 5) sink(s, 'The penny reached the far bank. Try a fresh run through all three rings.');
  },
  pointer(s, type) { if (type === 'down') tap(s); },
  action(s, id) { if (id === 'skip') tap(s); },
  key(s, k, down) { if (k === ' ' && down) tap(s); },
  draw(s, d) {
    for (const [i, w] of s.wells.entries()) {
      d.ellipse(w.x, w.y + 6, 62, 41, '#1d555544');
      d.ellipse(w.x, w.y, 62, 40, null, w.hit ? '#f4d990' : '#bd9768', 6);
      ...
      if (i === s.hits) d.glow(w.x, w.y, 65, '#ecd19a');
    }
    ...
    if (c) {
      ...
      d.item(spriteKey('everyday-penny'), c.x, c.y - c.z, {
        w: 32, fallback: () => { d.ellipse(c.x, c.y - c.z, 17, 6 + Math.abs(Math.cos(s.t * 14)) * 8, '#d5b077', '#f6d995', 2); d.star(c.x, c.y - c.z, 7, '#f6dfa1'); },
      });
      if (c.touch) d.text('SKIP', c.x, c.y + 49, 18, '#fff0b8');
    } else d.ring(450, 1025, 23, '#e6c48b', 9);
  },
