...
function throwBall(s) {
  ...
  const t = .85;
  s.ball = {x: 450, y: 1050, vx: (s.aim.x - 450) / t, vy: (s.aim.y - 1050 - 250 * t * t) / t};
  s.throws++;
}
...
export default {
  ...
  create(level) {
    return {
      level, t: 0, aim: {x: 265, y: 690}, ball: null, throws: 0, hit: 0, dunk: -1,
      latches: [{bx: 265, by: 690, x: 265, y: 690}, {bx: 640, by: 675, x: 640, y: 675}, {bx: 580, by: 465, x: 580, y: 465}],
      guard: {x: 450, y: 865}, note: 'The first brass latch is glowing.',
    };
  },
  update(s, dt, input) {
    s.t += dt;
    ...
    s.aim.x = clamp(s.aim.x + dx * 210 * dt, 210, 690);
    s.aim.y = clamp(s.aim.y + dy * 210 * dt, 410, 820);
    if (s.ball) {
  ...
  pointer(s, type, p) {
    if (type === 'move' || type === 'down') s.aim = {x: clamp(p.x, 210, 690), y: clamp(p.y, 410, 820)};
    if (type === 'up') throwBall(s);
  },
  action(s, id) { if (id === 'throw') throwBall(s); },
  key(s, k, down) { if (k === ' ' && down) throwBall(s); },
  draw(s, d) {
    const c = d.c;
    ...
    const y = s.dunk < 0 ? 610 : s.dunk < .65 ? 610 + 420 * s.dunk * s.dunk : 770 + Math.sin(s.dunk * 5) * 5;
    c.save(); c.beginPath(); c.rect(300, 400, 300, 420); c.clip();
    d.item(spriteKey('crowned-duck'), 450, y, {
      w: 110, fallback: () => d.animal(450, y, 'duck', 1.65, s.t),
    });
    d.ring(450, y + 10, 28, '#e7c786', 8);
    c.restore();
    d.ellipse(450, 817, 104, 24, '#70bab988');
    for (let i = 0; i < 8; i++) d.line({x: 355 + i * 27, y: 837}, {x: 355 + i * 27, y: 909}, '#91bdaf44', 2);
    for (const [i, l] of s.latches.entries()) {
      d.path([{x: l.x, y: l.y}, {x: l.x, y: 745}, {x: 360, y: 745}], i < s.hit ? '#eacb85' : '#997b55', 4);
      if (i === s.hit) d.glow(l.x, l.y, 50, '#e9c787');
      d.circle(l.x, l.y, 33, i < s.hit ? '#b9b985' : '#ba6c59', '#ead0a0', 4);
    ...
    if (s.ball) d.item(spriteKey('mercury-bead'), s.ball.x, s.ball.y, {w: 32, fallback: () => d.ball(s.ball.x, s.ball.y, 16, '#b49169')});
    else if (s.dunk < 0) {
      d.item(spriteKey('mercury-bead'), 450, 1050, {w: 38, fallback: () => d.ball(450, 1050, 19, '#c0a275')});
