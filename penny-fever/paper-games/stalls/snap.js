...
function shutter(s) {
  ...
  const target = s.animals[s.album.length];
  const inside = target && Math.abs(target.x - s.camera.x) < 83 && Math.abs(target.y - s.camera.y) < 57;
  if (inside && s.focus > .68) {
    ...
    s.note = inside
      ? 'Nearly! Hold a little steadier until the focus ring turns green.'
      : 'The subject slipped out of the frame. There is always another moment.';
    s.focus = 0;
  }
}
...
export default {
  ...
  update(s, dt, input) {
    ...
    s.camera.x = clamp(s.camera.x + dx * 220 * dt, 250, 650);
    s.camera.y = clamp(s.camera.y + dy * 220 * dt, 420, 950);
    const target = s.animals[s.album.length];
    const inside = target && Math.abs(target.x - s.camera.x) < 80 && Math.abs(target.y - s.camera.y) < 55;
    s.focus = clamp(s.focus + (s.holding && inside ? .85 : -1.5) * dt, 0, 1);
  },
  pointer(s, type, p) {
    if (type === 'cancel') { s.holding = false; s.focus = 0; return; }
    if (type === 'down' || type === 'move') {
      const delta = dist(s.camera, p);
      s.camera.x = clamp(p.x, 250, 650); s.camera.y = clamp(p.y, 420, 950);
      if (delta > 30) s.focus *= .7;
    }
    if (type === 'down') s.holding = true;
    if (type === 'up') shutter(s);
  },
  action(s, id, down) { if (id === 'focus') { if (down) s.holding = true; else shutter(s); } },
  key(s, key, down) { if (key === ' ') { if (down) s.holding = true; else shutter(s); } },
  draw(s, d) {
    for (const [i, a] of s.animals.entries()) {
      d.animal(a.x, a.y, a.kind, 1.6, s.t * 6);
      if (i === s.album.length) { d.star(a.x, a.y - 85, 10, '#e8c776'); d.text('NEXT', a.x, a.y - 103, 14, '#3c6253'); }
    }
    const p = s.camera, c = d.c;
    c.save();
    c.strokeStyle = s.focus > .68 ? '#bff4c4' : '#fff0c9'; c.lineWidth = 3;
    c.strokeRect(p.x - 106, p.y - 80, 212, 160);
    for (const x of [-35, 35]) d.line({x: p.x + x, y: p.y - 75}, {x: p.x + x, y: p.y + 75}, '#f8e3b577', 1);
    d.line({x: p.x - 100, y: p.y}, {x: p.x + 100, y: p.y}, '#f8e3b577', 1);
    ...
    for (let i = 0; i < 3; i++) {
      ...
      if (s.album[i]) {
        ...
        d.text(s.album[i].quality, x, 1106, 16, '#485345');
      } else {
        d.item(spriteKey('shutter-click'), x, 1068, {w: 36, alpha: .55, fallback: () => d.text('?', x, 1070, 28, '#85836b')});
      }
    }
