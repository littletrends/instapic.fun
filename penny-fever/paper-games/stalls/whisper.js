...
export default {
  ...
  instructions: 'Pull the letter back from the lower cloud and release. Match it to the glowing island letterbox. While flying, hold Left or Right to trim. Keyboard arrows steer; Space launches. The third chapter’s last delivery is a bottle, which drifts. Missed letters come back to you.',
  levels: ['The morning post', 'Crosswinds', 'The late-night express'],
  actions: [{id: 'left', label: 'Trim left', hold: true}, {id: 'launch', label: 'Launch letter'}, {id: 'right', label: 'Trim right', hold: true}],
  create(level) {
    return {level, t: 0, plane: {...perch, vx: 0, vy: 0}, aim: {x: 450, y: 1120}, drag: false, flying: false, delivered: 0, shots: 0, trail: [], wind: 0, note: 'First: the ' + names[0] + '.'};
  },
  update(s, dt, input) {
    s.t += dt;
    s.wind = Math.sin(s.t * .65) * (16 + s.level * 18);
    if (!s.flying) {
    ...
    const bottle = s.level === 2 && s.delivered === 2;
    ...
    const hit = boxes.findIndex(b => segmentDistance({x: b.x, y: b.y + Math.sin(s.t * .7 + b.x) * s.level * 10}, old, p) < 48);
    if (hit >= 0) {
      if (hit === s.delivered) {
        s.delivered++;
        if (s.delivered === 3) {
          done(s, 'Every little secret found a home', 'Three signed-and-sealed deliveries in ' + s.shots + ' flights. Willa is keeping the contents to herself.');
          return;
        }
        reset(s, 'Delivered. Now the ' + names[s.delivered] + (s.level === 2 && s.delivered === 2 ? ' — or rather, the bottle.' : '.'));
  ...
  draw(s, d) {
    for (const [i, b] of boxes.entries()) {
      const y = b.y + Math.sin(s.t * .7 + i) * s.level * 10;
    ...
    const p = s.plane, bottle = s.level === 2 && s.delivered === 2;
    const id = bottle ? 'message-bottle' : letters[Math.min(s.delivered, 2)];
    d.item(spriteKey(id), p.x, p.y, {w: bottle ? 54 : 62, fallback: () => {
      ...
      d.text(boxes[Math.min(s.delivered, 2)].symbol, 0, 4, 14, '#bd768f');
      c.restore();
    }});
  },
