...
Deployment: published as `13cfd44`; Pages run `34939642678` succeeded. Live Chromium phone checks passed real touch cancellation/launch and independent two-finger release, one launch debit, exact paused table and purse restoration through Treasures/reload, resumed first-prize award, no house timer, all six chapter controls and selected-chapter restore. Phone and desktop rendered without reported script or HTTP errors in this run.

## Pip timed impacts and bounded returns — 15 September 2026

Supersedes the earlier no-timer/first-basic-hit Pinball rule at the user's request. Each new ball gets 100 seconds of active play. Menus, return/reload and the shooter-lane wait preserve remaining time; relaunching the same ball cannot renew it. Timeout drains that ball, extinguishes the table, stops scoring and leaves prepaid balls usable. A new ball restores the lights and timer. The engine counts physics substeps; the runtime displays the saved remaining allowance without counting it twice. The cabinet counter shows elapsed seconds 1–100.

The chapter bonus is available on the first ball, but only a bumper impact in that chapter's listed whole winning second releases it. There is no guaranteed first-hit award or cumulative-hit fallback. Six tables have different bumper/target/pocket positions and distinct winning windows. Ordinary returns have lower impact probabilities, an eight-second interval, and a maximum of two pennies plus one token per ball; these limits persist across returns. Bonus awards remain separately deduplicated. Pocket release places the ball beyond obstructing bumpers, and capture only rearms after the ball reaches the lower field. Old active layouts finish in place and upgrade on the next ball, preserving paid credits, score and collection history.

Evidence: `check-pip-state.mjs` passes all 600 winning/non-winning second windows, six pocket ejections/re-entry locks, forced rapid-impact 100-second runs with bounded payouts, lights-out scoring cutoff, prepaid relaunch, exact timer/trajectory/RNG/limit restore, legacy obstructed pocket recovery and deferred layout migration. Existing practice and first-prize suites pass. Chromium at 390×844 passes actual touch launch/cancel, independent two-finger flippers, game-specific Treasures return and exact paused reload, a saved winning-impact fixture through the real reward flow, a saved near-timeout fixture through lights-out and prepaid relaunch, six chapter controls and selected-chapter reload. Phone lit/dark and desktop captures inspected. Browser impact/expiry checks use saved fixtures; this is not a human playthrough or a certified payout model. Unavailable storage, cross-device sync and atomic wallet/save writes remain outside this pass.

Deployment: code `5485ab8` published to main; GitHub Pages run `34945638706` succeeded. The same targeted Chromium phone test passed against https://instapic.fun: touch controls, exact saved return, winning-impact fixture award, timed lights-out, prepaid relaunch and chapter restore, with no reported script or HTTP errors. Work was validated in an isolated checkout after concurrent shared-file edits reverted part of the initial working change.

## Iris locked/unlocked copy and Copper 5-packs — 15 September 2026

Iris chapter 6 now awards the book’s Paper crown (Looking-glass locket stays Opal’s). Catching the rings is no longer labelled CAUGHT/KEPT. A clean catch that opens the chapter bonus reads UNLOCKED / “Bonus unlocked.” A clean catch that does not open it reads LOCKED / “Bonus locked — better luck next time.” A later catch on an already owned chapter reads “Already unlocked.” Missed rings still say NOT THIS CATCH. The 1–100 bonus gate and first included gaze are unchanged.

Copper adds 5-penny packs on the cabinet and in the alley menu. Packing spends five loose pennies and stores them out of the droppable purse; opening a pack returns five. Drop purse / ¼ / ½ only dump remaining loose pennies. Packs persist on the live wallet (`pennyPacks`) and in standalone practice storage. Existing v6 trays, first-paid-penny release and accounting are unchanged.
